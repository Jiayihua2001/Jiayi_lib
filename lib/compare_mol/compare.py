...
from ibslib.molecules.label import mol2graph,message_passing_atom_descriptors,\
                                    get_symmetric_ordering,label_mp,get_hash_order
from ibslib.molecules.align import get_principal_axes,fast_align
...
def compare(mol1, mol2, rmsd_tol=0.15, bonds_kw={}, chiral=True, fixed_order=[]):
    ...
    if ret_dups:
        ...
        return ret_dups,min_rmsd,min_trans,min_rot,min_idx
...
def compare_rmsd(mol1, mol2, rmsd_tol=0.15,  bonds_kw={}, chiral=True, fixed_order=[]):
    ...
    geo1 = mol1.get_geo_array()
    ele1 = mol1.geometry["element"]
    cmol1 = Structure.from_geo(geo1, ele1, struct_id="copy_mol1")
    cmol1.properties["bonds"] = mol1.properties["bonds"]
    
    geo2 = mol2.get_geo_array()
    ele2 = mol2.geometry["element"]
    cmol2 = Structure.from_geo(geo2, ele2, struct_id="copy_mol2")
    ...
    geo1 = cmol1.get_geo_array()
    geo2 = cmol2.get_geo_array()
...
def compare_rmsd_pa(mol1, mol2, 
    ...
    if not chiral:
        ...
        inv_cmol2 = Structure.from_geo(inv_geo2, ele2, struct_id="copy_inv_mol2")
...
def generate_combined(mol1,mol2,results):
    ...
    if results[5] > 0:
        ...
        ele2 = mol2.geometry["element"]
        cmol2 = Structure.from_geo(geo2, ele2, 
                            struct_id="inverse-{}".format(mol2.struct_id))
    ...
    temp_mol1 = Structure.from_geo(geo1,ele1,struct_id=mol1.struct_id)
    temp_mol2 = Structure.from_geo(geo2,ele2,struct_id=mol2.struct_id)
    
    comb_id = "Combined_{}_{}".format(mol1.struct_id,mol2.struct_id)
    comb_geo = np.vstack([geo1,geo2])
    comb_ele = np.hstack([ele1,ele2])
    comb = Structure.from_geo(comb_geo,comb_ele,struct_id=comb_id)
