...
from ibslib.molecules.label import label,match_labels
# try: 
...
class FindMolecules(BaseDriver_):
    ...
    inorganic: bool
        Controls if the behavior of FindMolecules should be modified for 
        inorganic, ligand structures. The only thing that changes is that the 
        mult_range is explored from largest to smallest whereas the mult 
        range for organic systems is varied from smallest to largest.
    mult_range: iterable
    ...
    def __init__(self, 
        ...
        self.formula_check = formula_check
        self.inorganic = inorganic
        self.order_mol = order_mol
    ...
    def calc_struct(self, struct):
        ...
        if self.residues == 0:
            ...
            step = 1
            if self.inorganic == True:
                if check_inorganic(struct):
                    step = -1
                else:
                    step = 1
    ...
    def _calc_struct(self, struct, mult=1.05):
        ...
        if not self.formula_check:
            ...
            self.unique = find_unique_molecules_formula(self.molecules)
    ...
    def _reorder_atoms(self):
        """
        Reorders atoms using ibslib label function and match_labels
        
        """
        if not self.order_mol:
            return
    ...
    def _reorder_atoms_old(self):
        """
        THIS IS DEPRECATED: The OpenBabel approach never worked well and 
        givens unneccessary warnings that will concern users. This has be 
        replaced in favor of my own algorithm.
...
def find_unique_molecules_formula(molecule_struct_list):
    """
    ...
    for mult in [0.85, 0.9, 0.95, 1.0, 1.1, 1.15, 1.2, 1.25, 1.3]:
        ...
        for molecule_group in same_formula:
            ...
            for molecule in molecule_group:
                fragments,count = bn.calc_struct(molecule)
...
### Don't want to improt from Structure folder because this can create problems
...
def check_inorganic(struct):
    ...
    metals = ['Ag', 'Al', 'As', 'Au', 'Ba', 'Be', 'Bi', 'Ca',
              ...
              'Ga', 'Gd', 'Ge', 'Hf', 'Hg', 'Ho', 'In', 'Ir', 'K', 'La',
              'Li', 'Lu', 'Mg', 'Mn', 'Mo', 'Nb', 'Nd', 'Ni', 'Np',
              'Os', 'Pb', 'Pd', 'Pr', 'Pt', 'Re', 'Rh', 'Ru', 'Sb',
              'Sc', 'Se', 'Si', 'Sm', 'Sn', 'Ta', 'Tb', 'Tc', 'Te', 'Th', 'Ti',
              'Tl', 'U', 'V', 'W', 'Y', 'Yb', 'Zn', 'Zr']
